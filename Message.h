...
typedef enum : unsigned char {
	C_PRESENTATION_CHILDREN		= 0, //!< Sent by a children node to the parent when they are presenting themselves, This is usually done at startup.
	C_PRESENTATION_PARENT			= 1, //!< Sent by a parent node to children regularly. This is usually done at startup and during a normal run at intervals.
	C_SET											= 2, //!< This message is sent from or to a sensor when a sensor value should be updated.
	C_REQ											= 3, //!< Requests a variable value (usually from an actuator destined for controller).
	C_SYSTEM									= 4, //!< Internal MySensors messages (also include common messages provided/generated by the library).
	C_STREAM									= 5, //!< For firmware and other larger chunks of data that need to be divided into pieces.
	C_ACK											= 6	 //!< Sensor command that send an ACK for the last received command
} Sensor_command;
...
typedef enum : unsigned char {
  ...
  V_POWER_FACTOR			= 56,	//!< S_POWER, Ratio of real power to apparent power: floating point value in the range [-1,..,1]
	V_MOTION			= 57				//!< Used for setting/reporting binary (on/off) status. 1=on, 0=off
...
typedef enum : unsigned char {
	...
	I_ACK	= 30, //!< system message type that goes in pair with C_ACK
	I_NEWVALUE = 31 //!< the sensor is sending a new value
} System_message_type;
...
typedef enum : unsigned char{
	...
	P_INT					= 2,	//!< Payload type is INT16
	P_UINT				= 3,	//!< Payload type is UINT16
	...
	P_ACK						= 9,	//!< Payload type is empty
	P_BOOL					= 10, //!< Payload type is bool
	P_CHAR					= 11, //!< Payload type is char
	P_UCHAR					= 12, //!< Payload type is char
	P_BYNARY_BYTE		= 13 //!< Payload type is char
